...
- **chrome.alarms** — 定時任務排程，SW 重啟後仍持久化
- **chrome.notifications** — 排程任務觸發通知
...
│   ├── storage/
│   │   └── templates.ts   # chrome.storage.local CRUD + scheduledTasks
│   ├── background/
│   │   └── service-worker.ts  # Alarm scheduling, SCHEDULE_TASK / CANCEL_TASK handlers
│   └── popup/
...
type PlatformKey = "grok" | "gemini" | "chatgpt" | "claude";

interface ScheduledTask {
  id: string;                     // crypto.randomUUID()
  name: string;                   // user-defined task name
  templateId: string;             // linked Template id
  platform: PlatformKey;          // target platform to open
  variables: Record<string, string>; // pre-filled variable values
  scheduleType: ScheduleType;
  scheduleTime: string;           // ISO string (once) or "HH:MM" (daily/weekly)
  weekdays: number[];             // 0=Sun ... 6=Sat (weekly only)
  enabled: boolean;
  autoSubmit: boolean;            // automatically submit after setText()
...
### Export Modes (管理頁面 → 匯出)
| Mode | 行為 |
|------|------|
| **全部匯出** | 單一 JSON 檔，含所有模板 |
| **依分類匯出** | 每個分類各自下載一個 JSON 檔 |
| **選取匯出** | 展開分類樹，勾選個別模板後匯出 |

### Import Modes (管理頁面 → 匯入)
| Mode | 行為 |
|------|------|
| **合併（保留衝突）** | 僅新增 ID 不存在的模板，重複 ID 分配新 UUID |
| **覆蓋（更新衝突）** | 相同 ID 以匯入版本覆蓋，不衝突的保留 |
| **取代全部** | 清空現有模板後匯入 |

## Scheduled Prompt Sending (定時發送)

### Architecture
- `chrome.alarms` 負責定時觸發（跨 SW 重啟持久化）
- SW 收到 alarm → `chrome.tabs.create({ url: PLATFORM_URLS[platform] })` → 等待 tab ready → 發 `SCHEDULED_PROMPT` 給 content script
- Content script adapter 執行 `setText()` + 可選 `submit()`
- 每次執行後計算 `nextRunAt` 並重新 schedule alarm

### Service Worker Messages
| Type | Payload | Action |
|------|---------|--------|
| `SCHEDULE_TASK` | `{ task: ScheduledTask }` | 計算 nextRunAt → 建立 chrome.alarm |
| `CANCEL_TASK` | `{ taskId: string }` | 清除 chrome.alarm |
| `SCHEDULED_PROMPT` | `{ content: string, autoSubmit: boolean }` | content script 填入並送出 |

### Schedule Logic (`src/background/service-worker.ts`)
- `once` — 指定 ISO 時間，執行後不重新排程
- `daily` — 每天 HH:MM，執行後 nextRunAt + 24h
- `weekly` — 選定星期幾 + HH:MM，執行後算下一個符合的星期
...
7. **chrome.alarms 排程** — 取代 setInterval，SW sleep 後也能喚醒執行定時任務
